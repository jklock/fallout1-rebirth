...
PATCHED_DIR="$ROOT_DIR/GOG/patchedfiles"
...
OPTIONS:
  ...
  --print-source           Print canonical patched data path and exit
...
if [[ ! -d "$PATCHED_DIR" ]]; then
  fail "Canonical patched data directory is missing: $PATCHED_DIR"
fi
if [[ ! -f "$PATCHED_DIR/master.dat" || ! -f "$PATCHED_DIR/critter.dat" || ! -d "$PATCHED_DIR/data" ]]; then
  fail "Canonical patched data is incomplete at $PATCHED_DIR (need master.dat, critter.dat, data/)"
fi
...
if [[ ! -f "$TARGET_RESOURCES/master.dat" || ! -f "$TARGET_RESOURCES/critter.dat" || ! -d "$TARGET_RESOURCES/data" ]]; then
  ...
  if [[ "$src_master_hash" != "$dst_master_hash" || "$src_critter_hash" != "$dst_critter_hash" ]]; then
    need_install=1
    reason="target DAT hashes do not match canonical patchedfiles"
  fi
fi

if [[ "$need_install" == "1" ]]; then
  log "Installing canonical patched data into app ($reason)"
...
if [[ "$src_master_hash" != "$verify_master_hash" || "$src_critter_hash" != "$verify_critter_hash" ]]; then
  fail "Target resources DAT hashes do not match canonical patchedfiles after install"
fi

log "Canonical patched data is installed and verified"
