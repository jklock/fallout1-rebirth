...
def main(argv: Optional[Sequence[str]] = None) -> int:
    repo = Path('.').resolve()
    patched = repo / 'GOG' / 'patchedfiles' / 'master.dat'
    unpatched = repo / 'GOG' / 'unpatchedfiles' / 'master.dat'
    ...
    out_entries = repo / 'development' / 'RME' / 'ARTIFACTS' / 'evidence' / 'gate-2' / f"{mapname.lower()}-dat-entries.txt"
    out_info = repo / 'development' / 'RME' / 'ARTIFACTS' / 'evidence' / 'gate-2' / f"{mapname.lower()}-extract-info.txt"
    target_map = repo / 'GOG' / 'patchedfiles' / 'data' / 'maps' / f"{mapname}.MAP"
