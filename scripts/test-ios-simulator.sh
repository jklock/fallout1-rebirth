...
#
# USAGE:
#   ./scripts/test-ios-simulator.sh              # Build + install + launch
...
detect_bundle_id() {
    ...
    if [[ ! -f "$plist" ]]; then
        ...
        log_info "Build the app first with: ./scripts/test-ios-simulator.sh --build-only"
        exit 1
    fi
...
list_ipad_simulators() {
    ...
    echo "  SIMULATOR_NAME='iPad Air (5th generation)' ./scripts/test-ios-simulator.sh"
}
...
launch_app() {
    ...
    echo "  ./scripts/test-ios-simulator.sh --shutdown"
}
...
main() {
    ...
    echo "system slowdowns, run: ./scripts/test-ios-simulator.sh --shutdown"
}
