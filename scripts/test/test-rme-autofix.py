...
Behavior:
- Load rme-run-summary.json if present, else run test-rme-parse-log.py on rme.log + work/rme-selftest.json.
- Load rules from `test-rme-autofix-rules.py` and run them to produce candidate fixes.
- Write per-iteration artifacts under <workdir>/fixes/iter-<i>/
- If --apply, update files in workdir (guarded by safe workdir prefix checks)
...
PARSER = os.path.join(REPO_ROOT, "scripts", "test", "test-rme-parse-log.py")
RULES_MODULE = os.path.join(REPO_ROOT, "scripts", "test", "test-rme-autofix-rules.py")


def _safe_apply_allowed(workdir: str, allow_any_workdir: bool) -> bool:
    if allow_any_workdir:
        return True

    abs_workdir = os.path.abspath(workdir)

    # Allow explicit prefix override for non-default temp roots.
    env_prefix = os.environ.get("RME_AUTOFIX_ALLOWED_WORKDIR_PREFIX", "").strip()
    if env_prefix:
        for raw_prefix in env_prefix.split(":"):
            prefix = raw_prefix.strip()
            if not prefix:
                continue
            if abs_workdir.startswith(os.path.abspath(prefix)):
                return True

    # Default safeguard: only operate on ephemeral run workdirs.
    return "/tmp/rme-run-" in abs_workdir or "rme-run-" in abs_workdir
...
def apply_patches_to_workdir(workdir: str, patches: List[Dict[str, Any]], allow_any_workdir: bool):
    if not _safe_apply_allowed(workdir, allow_any_workdir):
        raise RuntimeError(
            "Refusing to apply patches: workdir is outside allowed prefixes. "
            "Use --allow-any-workdir only when targeting disposable data."
        )
...
def main(argv):
    ...
    parser.add_argument(
        "--allow-any-workdir",
        dest="allow_any_workdir",
        action="store_true",
        help="Disable apply safety-prefix checks (dangerous; use only on disposable data).",
    )
    ...
    allow_any_workdir = args.allow_any_workdir
    ...
    for i in range(1, iterations + 1):
        ...
        if patches and do_apply and not dry_run:
            try:
                applied = apply_patches_to_workdir(workdir, patches, allow_any_workdir)
                # write applied diff
