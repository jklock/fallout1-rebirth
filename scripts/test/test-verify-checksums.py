#!/usr/bin/env python3
"""Verify checksums in third_party/rme/checksums.txt against a patched data overlay.
Writes a report listing missing files, mismatched hashes, ambiguous matches, and unexpected extras.

Usage:
  scripts/test/test-verify-checksums.py --checks third_party/rme/checksums.txt --overlay /path/to/patchedfiles --out /path/to/checksums-verify.txt
"""
import argparse
import hashlib
import os
...
with open(out, 'w', encoding='utf-8') as f:
    ...
    if ambiguous:
        ...
        f.write('\n')
    if extras:
        f.write('--- Unexpected extras found in overlay (not referenced in checksums) ---\n')
