...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    default_patched = os.environ.get("GAME_DATA")
    if not default_patched and gamefiles_root:
        default_patched = str(Path(gamefiles_root) / "patchedfiles")

    parser = argparse.ArgumentParser(description="Audit missing scripts and references (MAP/PRO)")
    parser.add_argument(
        "--patched-dir",
        default=default_patched or "",
        help="Patched dir with master.dat/critter.dat and data/",
    )
    ...
    if not args.patched_dir:
        print(
            "[ERROR] --patched-dir is required unless GAME_DATA or FALLOUT_GAMEFILES_ROOT is set.",
            file=sys.stderr,
        )
        return 2
