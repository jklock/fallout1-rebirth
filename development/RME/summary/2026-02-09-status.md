...
- Render instrumentation results: multiple `GNW_SHOW_RECT` events were observed where a surface is non-zero before a show rect (`surf_pre>0`) and zero after it (`surf_post==0`), frequently adjacent to `WIN_FILL_RECT` fills that target the same top rectangles. `RENDER_PRESENT_ANOMALY` capture was added to save presented frames for manual inspection (`/tmp/f1r-present-anom-*.bmp`).
