...
Subagent task files (automated, non-interactive)
- `development/RME/TODO/subagent_infrastructure.md` — build + patch + install + RME_SELFTEST (Gate‑1)
- `development/RME/TODO/subagent_maps.md` — flaky repeats + full 72‑map sweep (Gate‑2)
- `development/RME/TODO/subagent_art.md` — FRM/LST checks + anomaly screenshots
- `development/RME/TODO/subagent_scripts.md` — `RME_SELFTEST`, `test-rme-patchflow.sh`, autorun map checks
- `development/RME/TODO/subagent_ios.md` — iOS simulator build/install/launch (Gate‑4)
- `development/RME/TODO/subagent_audit.md` — whitelist apply dry‑run, placeholder audit, GNW anomaly triage

How to run a subagent task (example)
- Non-interactive, fully automated:
  1) Start with infra: run `./scripts/build/build-macos.sh` then `./scripts/patch/rebirth-patch-app.sh --base GOG/unpatchedfiles --out GOG/patchedfiles --force` and `./scripts/test/test-install-game-data.sh`.
  2) Execute the subagent file commands; e.g. to run the maps automation run the `maps` commands in `subagent_maps.md`.
  3) All outputs are written to `development/RME/ARTIFACTS/evidence/*` (CSV, patchlogs, screenshots). The subagent prompts in each `subagent_*.md` specify the exact JSON summary format to return.

Notes
- Each `subagent_*.md` includes an EXACT subagent prompt you can pass to an automation agent (or ask me to run). They are fully non-interactive and safe to run in CI or locally.
- If you want me to run any subagent now, tell me which one (I can run P0 CARAVAN repeats or the full infra + sweep flow).
